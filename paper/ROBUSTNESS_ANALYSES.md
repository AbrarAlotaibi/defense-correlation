# Robustness analyses: permutation inference, grader variance, external thresholds

Three robustness analyses were run on top of the primary result — Task A (inference on the
existing data), Task B (grader variance) and Task D (threshold transfer). Task C (attack-seed
variance) was not run; §"Task C" states why and what stands in its place. Every number below is
...
**11 of 21 pairs differ significantly after Holm** (14 uncorrected), and **Llama Guard
dominates every other member pairwise**: there is no behaviour on which Llama Guard is breached
while another defense holds.
...
**A4 is not computable, and the file records that** rather than returning an empty result. The
...
**Marginals under majority vote.** Undefended ASR: **0.63** adaptive (0.08 plain, 0.27
...
> **One expectation going in is contradicted.** The expectation was that measuring grader
> variance would let the manuscript **drop** the warning against ordering Δ values within 0.05.
> The measurement says keep it: of the 74 pairs-of-pairs whose Δ differs by less than 0.05, **67 (91%) have
...
Not run. The single-seed limitation remains stated accurately in Limitations.
...
**The answer is neither "yes" nor "no".** probe₁₆ × probe₈ keeps a strong
...
## Two confirmations on the judging counts
...
`paper/robustness_insert.tex` carries all of the above; the other inserts were edited to match.
